...
namespace YAXLib
{
    ...
    public class YAXSerializer
    {
        ...
        private object DeserializeBase(XElement baseElement)
        {
                ...
                if (m_exceptionOccurredDuringMemberDeserialization)
                {
                    if (m_desObject == null) // i.e. if it was NOT resuming deserialization, set default value, otherwise existing value for the member is kept
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //member.SetValue(o, member.DefaultValue /*the value to be assigned */);
                            }
                        }
                    }
                }
            ...
            //    return null;
            var F = GetFieldsToBeSerialized();
            MemberWrapper W = F.FirstOrDefault(X => {
                return X.CollectionAttributeInstance != null && X.CollectionAttributeInstance.SerializationType == YAXCollectionSerializationTypes.RecursiveWithNoContainingElement;
            });
            if (W != null)
            {
                foreach (XElement element in baseElement.Elements())
                {
                    ...
                    Type CommonType = CommonTypes
                        .FirstOrDefault(X =>
                        {
                            object A = X.GetCustomAttributes(typeof(YAXSerializeAsAttribute), false).FirstOrDefault();
                            if (A != null)
                                return (X.IsClass && (X.IsSubclassOf(W.MemberType.GetGenericArguments()[0]) || X == W.MemberType.GetGenericArguments()[0]) && (A as YAXSerializeAsAttribute).SerializeAs == element.Name);
                            else return false;
                        });
                    if (CommonType != null)
                    {
